...
st.set_page_config(page_title="Soybean Disease Diagnosis", page_icon="🌱", layout="wide")
...
@st.cache_data
def load_data():
    return pd.read_csv('soybean_disease_dataset (1).csv')
...
def load_pipeline_and_model():
    pipeline = joblib.load('soybean_pipeline.pkl')
    model = load_model('soybean_disease_model.keras')
    return pipeline, model

pipeline, model = load_pipeline_and_model()

st.title("🌱 Soybean Disease Diagnosis Prediction")
st.markdown("---")
...
st.write("""
🌿 This tool uses a deep learning model to predict soybean diseases based on various plant characteristics. Choose the features in the sidebar and click '🔍 Predict Disease' to get a diagnosis. The confidence scores show how confident the model is about each possible disease class. Keep your plants healthy! 💪
""")

st.markdown("---")
st.markdown("Created  by DARSHAN JEBARAJ.D ")
